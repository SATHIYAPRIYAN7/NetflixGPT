...
function Moreinfo({infom,showinfo}) {
 const selector =useSelector(store => store.Movies?.hero)
  ...
  return (
    ...
    <img className='w-full h-56 mt-4 px-20 md:h-auto md:mt-0 md:px-0  sm:w-auto' src={Poster_Path +selector?.poster_path}/>
    <div className='mt-8  pl-2 md:mt-20 pr-2'>
        <h1 className='text-white text-xl md:text-base font-bold'>{selector.original_title}</h1>
